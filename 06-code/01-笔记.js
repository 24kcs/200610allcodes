 ...
 */
  ...
  * 
  * Vue中如何发送异步请求的操作
  * 1) vue-resource---以前使用的方式
  * 2) axios----现在推荐使用的方式--(请求拦截器和响应拦截器)
  * vue-resource和axios 都是插件
  * 需要先下载安装 
  * npm install vue-resource
  * npm install axios
  * 
  * vue-resource插件,需要在main.js中引入,声明使用插件,每个组件的对象this中就会出现一个$http的属性
  * axios 需要在发送请求的组件或者文件中,引入,通过axios对象.相关的方法
  * 
  * Vue中的路由:用来实现单页面应用开发的(SPA)
  * 路由器:用来管理多个路由对象
  * 路由:指的是一种映射关系,地址和组件组成
  * 如果要使用路由,首先要下载安装路由器插件
  * npm install vue-router
  * 声明使用插件
  * 实例化路由器的对象,并暴露出去
  * 该路由器对象内部需要进行路由的注册,也可以设置浏览器的地址栏中是否出现#
  * mode:'hash' 地址栏带#
  * mode:'history' 地址栏不带#
  * 所有的路由的注册都在route数组中进行,每个路由都是一个对象
  * 每个路由对象中,有path,是地址,有component,是组件
  * 普通的组件一旦通过路由进行注册后,该组件就是路由组件
  * 一旦路由注册成功后,每个组件对象中都会有两个对象:
  * $route:路由信息对象,内部有path属性用来获取当前路由跳转的地址,params对象和query对象
  * 路由跳转的时候可以有params的方式进行传参,也可以有query的方式进行传参
  * 如果在注册路由的时候,path地址中有'/:名字' 名字就是占位,是属于params的方式进行传参
  * $router:路由器对象,可以调用push/replace/back方法,进行编程式路由
  * 声明式路由:通过<router-link to="地址" />和<router-view/> 进行路由的操作(跳转和传参)
  * <router-link> 路由链接,最终在页面中以a标签进行呈现
  * <router-view> 路由视图,最终显示的是对应的组件内容
  * 编程式路由:是通过js代码的方式进行实现,使用的是push/replace/back...
  * 路由视图也可以进行传参
  * <keep-alive/> 可以进行组件的缓存
  * 路由:可以分为一级路由/二级路由/三级路由....
  * 
  * 
  * 
  * vuex和MVVM源码分析,
