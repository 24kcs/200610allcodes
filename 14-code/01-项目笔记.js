 ...
 /*
  ...
  5.分类信息列表需要进行高亮显示/对应菜单展示和隐藏的操作(函数的节流),也有函数的防抖
    ...
    6. 制作组件使用的文档
      ...
      没有登录的情况
    调用接口的时候,需要携带用户的临时id凭证(标识)
    1. 先安装 npm install uuidjs 插件
    2. 通过该插件对象的相关方法产生临时id凭证
    3. 缓存到浏览器中,从浏览器中读取出来存储到vuex中(vuex的读取速度超过了浏览器的缓存)
    4. 在调用接口发送请求的时候,在请求头中携带该临时的用户id凭证即可
