            ...
            <div class="chooseArea">
              ...
              <dl v-for="(attr, index) in spuSaleAttrList" :key="attr.id">
                ...
                <dd
                  ...
                  v-for="(spuVal, index) in attr.spuSaleAttrValueList"
                  :key="spuVal.id"
                  @click="setChecked(spuVal, attr.spuSaleAttrValueList)"
                >
...
export default {
  ...
  data() {
    return {
      skuNum: 1, // 该产品默认的数量
      currentImageIndex: 0, // 用来存储图片轮播图中选中的某个图片的索引
    }
  },
  computed: {
    ...mapGetters([
  ...
  methods: {
    ...
    // 子级组件ImageList中点击某个小图,该图的索引,传递过来
    changeCurrentIndex(index) {
      this.currentImageIndex = index
    },
    // 点击加入购物车,跳转到添加购物车成功的界面
    async addToCart() {
      // 准备query参数,有skuId和skuNum
      const query = {
        skuId: this.$route.params.skuId,
        skuNum: this.skuNum,
      }
      // 分发action,调用接口,有可能不成功
      // 方式1: 使用回调函数
      // this.$store.dispatch('addToCart1', { ...query, callback: this.callback })
      // 方式2: 使用promise,因为dipatch('action的名字')的返回值是对应的action的返回值,是promise对象
      // 如果成功了,返回的是成功的promise的value值,如果内部失败了,则返回的是失败的promise
      // 可以使用async + await 简化写法
      const errorMsg = await this.$store.dispatch('addToCart2', query)
      // 直接判断返回结果
      if (!errorMsg) {
        // 通过浏览器缓存存储当前的商品信息数据对象
