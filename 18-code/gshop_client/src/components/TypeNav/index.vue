  ...
  methods: {
    ...
    toSearch(e) {
      ...
      if (nodeTarget.nodeName === 'A') {
        ...
        // 点击分类信息路由跳转的时候,当前的地址中是否包含/search,如果包含了/search,那么此时如果有params参数,那么应该和query参数一起携带,否则只需要携带query参数即可

        // 从路由信息对象中获取path地址和params参数
        const { path, params } = this.$route
        if (path.indexOf('/search') === 0) {
          // 上面的相对来讲会容易理解一点
          this.$router.replace({ path, query, params })
          // 或者
          // 此时的path中包含了params的参数,所以,可以直接传入query
